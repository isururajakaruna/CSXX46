...
from ats.strategies.simple_strategy.strategy import Strategy
...
with open('tests/configs/binance_test_exchange_config.yaml', 'r') as file:
    ...
    exchange_config = yaml.safe_load(file)['exchange']['config']
    strategy_config = yaml.safe_load(file)['strategy']['config']

exchange = Exchange(config=exchange_config)
...
def test_simple_market_order():
    ...
    order_1 = Order(order_side='BUY', order_type='MARKET',
                    size=0.001,
                    base_symbol=exchange_config['symbol']['base'],
                    quote_symbol=exchange_config['symbol']['quote']
                    )
...
def test_submitting_limit_orders_and_cancelling():
    ...
    print(f'Waiting for current close price of {exchange_config["symbol"]["base"]}')
...
def test_on_candle_callback():
    ...
    def on_candle_counter(candle):
        ...
        if isinstance(candle, Candle):
            expected_symbol = exchange_config['symbol']['base'] + exchange_config['symbol']['quote']
