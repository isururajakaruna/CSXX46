...
class Strategy(BaseStrategy):
    ...
    def __init__(self, config, state: SimpleState = None):
        ...
        # NEW: Track order value for transaction cost calculation (Phase 1)
        self.last_order_value = 0.0
    ...
    def _execute_action(self, action: int, candle: Candle):
        ...
        if action == self.ACTION_HOLD:
            self.last_order_value = 0.0  # No trade, reset order value
            return
        ...
        if action == self.ACTION_BUY:
            ...
            if self.quote_asset_balance >= order_value:
                # NEW: Track order value for transaction cost (Phase 1)
                self.last_order_value = order_value
            ...
            if self.base_asset_balance >= order_size and order_size > 0:
                # NEW: Track order value for transaction cost (Phase 1)
                self.last_order_value = order_size * self.current_price
    ...
    def _calculate_reward(self) -> float:
        """
        Enhanced reward function with transaction costs and hold incentive (Phase 1).
        
        Penalties:
        - Transaction cost: 0.1% of order value (applied once per trade)
        
        Bonuses:
        - Hold incentive: Small bonus for staying in position (reduces overtrading)
        ...
        # Base reward: Pure equity change
        reward = current_equity - self.previous_equity
        
        # Transaction cost penalty (applied ONCE per trade)
        if self.last_order_value > 0:
            transaction_cost = self.last_order_value * 0.001  # 0.1% of order value
            reward -= transaction_cost
            self.last_order_value = 0  # Reset after applying
        
        # Hold incentive (encourage position holding, reduce churn)
        if self.previous_action == self.ACTION_HOLD and self.base_asset_balance > 0:
            reward += 0.005  # Small bonus (~0.5% of typical equity)
